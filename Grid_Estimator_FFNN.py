...
# ==========================================
# 1. PHYSICS CONSTANTS (For CRLB)
# ==========================================
# Must match C++ Simulation parameters exactly
TX_POWER_DBM = 43.0
FREQ_GHZ = 2.1
N_RB = 25
NUM_REF_SIGNALS = N_RB * 2
FSPL_CONST = 32.44

# Noise Parameters (Watts)
BW_HZ = 5000000.0
NOISE_FIG_DB = 7.0
THERMAL_DBM = -174.0
NOISE_FLOOR_DBM = THERMAL_DBM + 10 * np.log10(BW_HZ) + NOISE_FIG_DB
# Convert dBm to Watts (10^((dBm-30)/10))
NOISE_MEAN_WATTS = 10 ** ((NOISE_FLOOR_DBM - 30) / 10.0) 
# Variance sigma^2 (Assuming Sigma = Mean from your C++ code)
NOISE_VARIANCE = NOISE_MEAN_WATTS**2 

# Calculate A_k (Signal Amplitude Constant at 1m)
# Formula: P_rx = A_k / d^2  => A_k = P_tx_linear / (Loss_factors)
const_db_part = TX_POWER_DBM - 10 * np.log10(NUM_REF_SIGNALS) - 20 * np.log10(FREQ_GHZ) - FSPL_CONST
A_K = 10 ** ((const_db_part - 30) / 10.0)
...
def preprocess_data(json_data):
    ...
    features = np.array(df.drop(['UE_ID', 'Sample_ID', 'X_Position', 'Y_Position', 'Closest_Grid_X', 'Closest_Grid_Y', 'Serving_PCI', 'GridID'], axis=1))
    ...
    return features, labels, grid_id, df, shuffled_original_indices
    #return features, labels, grid_id, df


def calculate_crlb_for_test_set(json_data, test_indices, n_samples=5):
    """
    Calculates the Cramér-Rao Lower Bound (RMSE) for the specific UEs in the test set.
    """
    crlb_values = []
    
    for idx in test_indices:
        ue = json_data['UEs'][idx]
        
        # 1. Get True Position
        x_ue = ue['position']['x']
        y_ue = ue['position']['y']
        
        # 2. Fisher Information Matrix Elements
        J_xx = 0.0
        J_yy = 0.0
        J_xy = 0.0
        
        # 3. Identify Visible Towers
        # Geometry is static across samples, so we look at sample 0
        first_sample = ue['samples'][0] if 'samples' in ue else {'servingCell': ue['servingCell'], 'neighborCells': ue.get('neighborCells', [])}
        
        visible_towers = []
        if 'servingCell' in ue:
            visible_towers.append(ue['servingCell'])
        elif 'servingCell' in first_sample:
             visible_towers.append(first_sample['servingCell'])
             
        if 'neighborCells' in ue:
            visible_towers.extend(ue['neighborCells'])
        elif 'neighborCells' in first_sample:
            visible_towers.extend(first_sample['neighborCells'])
            
        # 4. Sum Information from Towers
        for tower in visible_towers:
            x_bs = tower['x']
            y_bs = tower['y']
            
            dist_sq = (x_ue - x_bs)**2 + (y_ue - y_bs)**2
            if dist_sq == 0: continue
            dist_8 = dist_sq ** 4 # d^8
            
            # FIM Term: (4 * A^2) / (sigma^2 * d^8)
            Q_k = (4 * (A_K**2)) / (NOISE_VARIANCE * dist_8)
            
            J_xx += Q_k * (x_ue - x_bs)**2
            J_yy += Q_k * (y_ue - y_bs)**2
            J_xy += Q_k * (x_ue - x_bs) * (y_ue - y_bs)
            
        # 5. Time Diversity (N Samples)
        J_xx *= n_samples
        J_yy *= n_samples
        J_xy *= n_samples
        
        # 6. Invert Matrix
        det_J = (J_xx * J_yy) - (J_xy**2)
        
        if det_J <= 1e-20:
            crlb_values.append(float('inf')) # Singular geometry
        else:
            # Trace of Inverse Matrix
            crlb_variance = (J_xx + J_yy) / det_J
            crlb_values.append(math.sqrt(crlb_variance))
            
    return np.array(crlb_values)
...
class GridDataPreprocessor:
    ...
    def process_data(self) -> Dict[str, torch.Tensor]:
        ...
        grid_centers = all_grids_tensor[..., :2]
        grid_ids = all_grids_tensor[..., 2]
        return {
            'coordinates': all_grids_tensor,
...
if __name__ == "__main__":
    ...
    # Preprocessing - Returns shuffled indices to track UEs
    features, labels, grid_number, df, shuffled_indices = preprocess_data(json_data)
    ...
    # Identify Test UEs for CRLB calculation
    test_indices_original = shuffled_indices[train_size:]
    ...
    print("\n--- Calculating Theoretical Lower Bound (CRLB) ---")
    crlb_rmse_values = calculate_crlb_for_test_set(json_data, test_indices_original)
    
    # Filter out potential infinite values (singular matrices)
    valid_crlb = crlb_rmse_values[crlb_rmse_values != float('inf')]
    avg_crlb = np.mean(valid_crlb)
    print(f"Average Theoretical CRLB (RMSE): {avg_crlb:.4f} meters")
    print(f"Model Mean Error: {mean_error:.4f} meters")
    ...
    with open(path_directory + 'results_single.txt', 'w') as f:
        ...
        f.write(f"Theoretical CRLB: {avg_crlb:.2f} meters\n")
    ...
    # --- Plot IEEE Standard CDF (Model vs CRLB) ---
    plt.figure(figsize=(10, 6))
    
    # 1. ML Model CDF
    sorted_errors = np.sort(distance_error)
    cdf_ml = np.arange(1, len(sorted_errors) + 1) / len(sorted_errors)
    plt.plot(sorted_errors, cdf_ml, label="Proposed FFNN", color="blue", linewidth=2)
    
    # 2. CRLB CDF (Theoretical Limit)
    sorted_crlb = np.sort(valid_crlb)
    cdf_crlb = np.arange(1, len(sorted_crlb) + 1) / len(sorted_crlb)
    plt.plot(sorted_crlb, cdf_crlb, label="Theoretical Lower Bound (CRLB)", 
             color="black", linestyle="--", linewidth=2)
    
    plt.xlabel("Positioning Error (meters)", fontsize=12)
    plt.ylabel("Cumulative Probability (CDF)", fontsize=12)
    plt.title(f"Localization Performance vs. CRLB ({input_grid_size}m Grid)", fontsize=14)
    plt.legend(fontsize=11)
    plt.grid(True, which='both', linestyle='--', alpha=0.6)
    plt.xlim(0, percentile_90 * 1.5) # Focus on relevant range
    plt.savefig(path_directory + 'error_cdf_with_crlb.pdf')
