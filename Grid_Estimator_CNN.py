...
weight_decay = 1e-4
hidden_size = 4096
input_grid_size = 100# 10m grid size
...
path_directory = f"finalmodels/CNN/Grid_{input_grid_size}m/batch_size_{batch_size}/initial_lr_{lr}/epochs{num_epochs}_p{patience}_hs{hidden_size}/"
...
def preprocess_data(json_data):
    ...
    # Reshape features for CNN input (batch_size, channels, height, width)
...
class CNNPositioningModel(nn.Module):
    def __init__(self, input_channels, hidden_size=1024):
        super(CNNPositioningModel, self).__init__()
        
        # CNN layers
        self.conv_layers = nn.Sequential(
            nn.Conv2d(1, hidden_size // 32, kernel_size=(input_channels, 1), padding=(1, 0)),
            nn.BatchNorm2d(hidden_size // 32),
            nn.ReLU(),
            nn.Conv2d(hidden_size // 32, hidden_size // 16, kernel_size=(3, 1), padding=(1, 0)),
            nn.BatchNorm2d(hidden_size // 16),
            nn.ReLU(),
            nn.Conv2d(hidden_size // 16, hidden_size // 8, kernel_size=(3, 1), padding=(1, 0)),
            nn.BatchNorm2d(hidden_size // 8),
            nn.ReLU(),
            nn.Conv2d(hidden_size // 8, hidden_size // 4, kernel_size=(3, 1), padding=(1, 0)),
            nn.BatchNorm2d(hidden_size // 4),
            nn.ReLU(),
            nn.Conv2d(hidden_size // 4, hidden_size // 2, kernel_size=(3, 1), padding=(1, 0)),
            nn.BatchNorm2d(hidden_size // 2),
            nn.ReLU(), 
            nn.Conv2d(hidden_size // 2, hidden_size, kernel_size=(3, 1), padding=(1, 0)),
...
def train_model(model, train_loader, val_loader, num_epochs=1000, patience=500, learning_rate=1e-4, path_directory=None):
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            if path_directory:
                model_path = path_directory + 'best_cnn_model.pth'
...
if __name__ == "__main__":
    ...
    input_channels = features.shape[2]  # Number of features
    model = CNNPositioningModel(input_channels, hidden_size)
    ...
    dummy_input = torch.randn(1, 1, input_channels, 1).to(device)
    ...
    model = CNNPositioningModel(input_channels, hidden_size)
    model.load_state_dict(torch.load(path_directory + 'best_cnn_model.pth'))
    model.to(device)
    model.eval()
    ...
    plt.title(f"CDF of Distance Errors for {input_grid_size}m grid size CNN (Single Run)")
    plt.legend()
    plt.grid()
    ...
    # plt.xlabel("Distance Error (meters)")
    # plt.ylabel("Cumulative Probability")
    # plt.title(f"CDF of Distance Errors for {input_grid_size}m grid size CNN ({num_runs} runs)")
    # plt.legend()
    # plt.grid()
